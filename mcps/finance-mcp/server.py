...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="finance-mcp MCP Server",
...
示例:
  ...
  python server.py --list
    ...
    parser.add_argument("--port", "-p", type=int, default=8003, help="监听端口")
...
if __name__ == "__main__":
    ...
    if args.list:
        ...
        async def _list():
            ...
            print(f"=== finance-mcp 共 {len(tools)} 个工具 ===")
            print()
            for t in tools:
                print(f"  {t.name}")
                if t.description:
