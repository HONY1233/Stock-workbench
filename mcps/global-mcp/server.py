...
# 加载自定义源
from sources.international import register as register_international

_ = register_international(mcp)
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="global-mcp MCP Server",
...
示例:
  ...
  python server.py --list
    ...
    parser.add_argument("--port", "-p", type=int, default=8006, help="监听端口")
...
if __name__ == "__main__":
    ...
    if args.list:
        ...
        async def _list():
            ...
            print(f"=== global-mcp 共 {len(tools)} 个工具 ===")
            print()
            for t in tools:
                print(f"  {t.name}")
                if t.description:
